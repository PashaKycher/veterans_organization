...
export const createAnalytical = async (req, res) => {
    try {
        ...
        res.status(201).json({ success: true, token, message: "Матеріал створено" });
    } catch (error) {
...
export const updateAnalytical = async (req, res) => {
    try {
        ...
        const token = generateSessionToken(user._id);
        return res.status(200).json({ success: true, token, message: "Матеріал оновлено" });
    } catch (error) {
